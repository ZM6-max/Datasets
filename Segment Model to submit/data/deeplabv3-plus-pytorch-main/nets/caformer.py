...
def caformer_m36_in21ft1k(pretrained=False, **kwargs):
    model = MetaFormer(
        ...
        **kwargs)    
    if pretrained:
        msg = model.load_state_dict(torch.load('model_data/caformer_m36_384_in21ft1k.pth', map_location=lambda storage, loc: storage), strict=False)
        print(msg)
    return model
...
class channels_trans(nn.Module):     
    def __init__(self):
        super().__init__()
    def forward(self, x):
...
class MetaFormer(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        self.feature_maps = None    # 新增一个属性来保存特征图
        ...
        self.channels_trans = channels_trans()
