...
if __name__ == "__main__":
    ...
    #------------------------------------------------------------------#
    if backbone =="MixTransformer" or backbone =="Caformer" or backbone =="SegNext":
    ...
    if True:
        ...
        #-------------------------------------------------------------------#
        nbs             = 16
        lr_limit_max    = 2e-4 if optimizer_type == 'adam' else 1e-1
        lr_limit_min    = 2e-4 if optimizer_type == 'adam' else 1e-4
        ...
        # print(name)
            if 'backbone' in name:
                if param.dim() == 1:
                    params.append(
                        {"params":param, "weight_decay": 0, "lr" : Init_lr_fit})
                else:
                    params.append(
                        {"params":param, "weight_decay": weight_decay, "lr" : Init_lr_fit})
            else:
                if param.dim() == 1:
                    params.append(
                        {"params":param, "weight_decay": 0, "lr" : 2*Init_lr_fit})
                else:
                    params.append(
                        {"params":param, "weight_decay": weight_decay, "lr" : 2*Init_lr_fit})
        optimizer = {
            'adam'  : optim.Adam(params, Init_lr_fit, betas = (momentum, 0.999), weight_decay = weight_decay),
            'adamw' : optim.AdamW(params, Init_lr_fit, betas=(0.9, 0.999), eps=1e-8, weight_decay=weight_decay),
            'sgd'   : optim.SGD(params, Init_lr_fit, momentum = momentum, nesterov=True, weight_decay = weight_decay)
        }[optimizer_type]
