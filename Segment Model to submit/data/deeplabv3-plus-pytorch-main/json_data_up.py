...
class DataAugmentForObjectDetection():
    ...
    def __init__(self, change_light_rate=0.5,
                 ...
                 is_addNoise=False, is_changeLight=True, is_random_point=False, is_shift_pic_bboxes=False,
        ...
        for file in files:
            if file.endswith('jpg') or file.endswith('png'):
                ...
                if not os.path.exists(pic_path):
                    print(f"Skipping {file} as image file not found.")
                    continue

                # 检查 JSON 文件是否存在
                if not os.path.exists(json_path):
                    print(f"Skipping {file} as JSON file not found.")
                    continue
